...
# scp -r /Users/alvin/Documents/projects/startup-pwm/* user@your-vps-ip:/var/www/estafee/
```
...
# Start backend
pm2 start apps/backend/dist/main.js --name "estafee-backend" --env production

# Start frontend
pm2 start apps/frontend/.next/standalone/server.js --name "estafee-frontend" --env production --cwd /var/www/estafee/apps/frontend

# Or use the ecosystem config (recommended - see ecosystem.config.js)
...
sudo cp /var/www/estafee/apps/backend/.env.production ~/backup/
sudo cp /var/www/estafee/apps/frontend/.env.production ~/backup/

# Backup Nginx config
sudo cp /etc/nginx/sites-available/estafee.id ~/backup/

# Backup PM2 config
cp /var/www/estafee/ecosystem.config.js ~/backup/
```
