...
// ⚠️ PASTIKAN GANTI INI DENGAN ID USER UUID v4 YANG VALID DARI DATABASE KAMU
const LOGGED_IN_USER_ID = "9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d"; 
...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "add_item" | "manage_items">("manage_items");
  ...
  const [editingItemId, setEditingItemId] = useState<string | null>(null); // Jika ada ID, berarti mode Edit
  const [itemForm, setItemForm] = useState({
    title: "",
    description: "",
    pricePerDay: 0,
    condition: "GOOD", 
    images: "", // String dipisah koma
    stock: 1
  });
  ...
  const fetchUserData = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/api/users/${LOGGED_IN_USER_ID}`);
      ...
      setMessage({ text: "Gagal memuat data. Pastikan Server nyala & ID User benar.", type: "error" });
    }
  };
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ text: "Profile berhasil diperbarui!", type: "success" });
    } catch (err: any) {
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        // Ubah string "url1, url2" jadi array ["url1", "url2"]
        images: itemForm.images.split(",").map((url) => url.trim()).filter((url) => url !== ""),
      ...
      setMessage({ 
        text: editingItemId ? "Barang berhasil diupdate!" : "Barang berhasil ditambahkan!", 
        type: "success" 
      });

      // Reset Form & Refresh Data
      resetItemForm();
      fetchUserData(); // Refresh list barang
  ...
  const handleEditClick = (item: Item) => {
    ...
    setItemForm({
      ...
      images: item.images.join(", "), // Array to String
    });
    setActiveTab("add_item"); // Buka tab form
    setMessage(null);
  };
  ...
  const resetItemForm = () => {
    ...
    setItemForm({ title: "", description: "", pricePerDay: 0, condition: "GOOD", images: "", stock: 1 });
  };
    ...
    <div className="min-h-screen bg-gray-50 pt-28 pb-12">
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-2">Kelola profil, tambah produk, dan atur stok barangmu.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="w-full md:w-1/4 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col items-center sticky top-24">
            ...
            <div className="w-full space-y-2">
              <button
                onClick={() => { setActiveTab("manage_items"); resetItemForm(); }}
              ...
              <button
                onClick={() => { setActiveTab("add_item"); resetItemForm(); }}
                className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-all duration-200 flex items-center gap-3 ${
                  activeTab === "add_item"
                    ? "bg-indigo-600 text-white shadow-md shadow-indigo-200"
                    : "bg-gray-50 text-gray-600 hover:bg-gray-100"
              ...
              <button
                onClick={() => { setActiveTab("profile"); resetItemForm(); }}
                className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-all duration-200 flex items-center gap-3 ${
                  activeTab === "profile"
                    ? "bg-indigo-600 text-white shadow-md shadow-indigo-200"
                    : "bg-gray-50 text-gray-600 hover:bg-gray-100"
          ...
          <div className="w-full md:w-3/4">
            ...
            {activeTab === "manage_items" && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 animate-fade-in-up min-h-[500px]">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                  <h3 className="text-xl font-bold text-gray-800">Barang Saya ({userItems.length})</h3>
                  <button 
                ...
                {userItems.length === 0 ? (
                  <div className="text-center py-20 flex flex-col items-center">
                    ...
                    <p className="text-gray-400 text-sm">Mulailah dengan menambahkan barang pertamamu.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {userItems.map((item) => (
                      <div key={item.id} className="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow bg-white flex flex-col">
                        ...
                        <div className="p-4 flex flex-col flex-grow">
                          ...
                          <p className="text-indigo-600 font-bold mb-4">
                            Rp {parseInt(item.pricePerDay as any).toLocaleString('id-ID')}/hari
                          </p>
            ...
            {activeTab === "add_item" && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 animate-fade-in-up">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                  ...
                  {editingItemId && (
                    <button onClick={handleCancelEdit} className="text-sm text-red-500 hover:text-red-700 font-medium">
                      Batal Edit
                    </button>
                  )}
                </div>
                ...
                <form onSubmit={handleSaveItem} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      <label className="text-sm font-semibold text-gray-700">Harga (Rp/Hari)</label>
                      <input
                        type="number"
                        required
                        min="0"
                        value={itemForm.pricePerDay}
                        onChange={(e) => setItemForm({ ...itemForm, pricePerDay: Number(e.target.value) })}
                        className="w-full px-4 py-3 rounded-xl bg-gray-50 border-gray-200 border text-gray-900 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none"
                      />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <option value="FAIR">Layak (Fair)</option>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-gray-700">URL Gambar (Pisahkan dengan koma)</label>
                    <input
                      type="text"
                  ...
                  <div className="pt-4 flex justify-end gap-3">
                    ...
                    >
                      {loading ? "Menyimpan..." : (editingItemId ? "Update Barang" : "Simpan Barang")}
                    </button>
                  </div>
                </form>
              </div>
            ...
            {activeTab === "profile" && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 animate-fade-in-up">
                <h3 className="text-xl font-bold text-gray-800 mb-6 pb-4 border-b border-gray-100">Informasi Pribadi</h3>
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-gray-700">Bio Toko / Profil</label>
                    <textarea
                      rows={3}
